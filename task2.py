...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr, fc_x, fc_y, tc_x, tc_y, jet_x, jet_y):
        ...
        self.jet_x = jet_x
        self.jet_y = jet_y
        self.Delta_C3 = -0.05  # 5% cost reduction
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while 1:
            ...
            for i, _ in enumerate(self.motion):
                ...
                if self.calc_grid_position(node.x, self.min_x) in self.jet_x:
                    if self.calc_grid_position(node.y, self.min_y) in self.jet_y:
...
def main():
    ...
    grid_size = 1
    robot_radius = 1.0
        # Define a rectangular jet area laterally across the map
    jet_x, jet_y = [], []
    for i in range(0, 51):  # x range (horizontal span)
        for j in range(30, 35):  # y range (vertical width = 5 units)
            jet_x.append(i)
            jet_y.append(j)
    ...
    if show_animation:
        ...
        plt.plot(tc_x, tc_y, "or")
            # Draw jet area as a transparent light blue rectangle
    jet_rect = plt.Rectangle((0, 30), 51, 5, color='skyblue', alpha=0.3)
    plt.gca().add_patch(jet_rect)


        plt.grid(True)
        plt.axis("equal")

    a_star = AStarPlanner(ox, oy, grid_size, robot_radius, fc_x, fc_y, tc_x, tc_y, jet_x, jet_y)
