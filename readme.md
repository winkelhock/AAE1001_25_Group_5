...
_(An image showing jet stream initialisation, VS Code)_
...
_(An image showing optical loop placement, VS Code)_
...
_(An image showing the Cost Calculation logic code, VS Code)_
...
_(An image showing the code performing cost analysis, VS Code)_
...
_(An image showing optimal jet stream placement as per the parameters, VS Code)_
...
_(An image showing final code analysis results, VS Code)_

With the help of GenAI, we were able to refine our coding procedures, ensuring cleaner outputs and more efficient prompt methods, resulting in optimal results.
...
#### Description
In reality, the flight path is not fixed for every single flight every time. It is dynamic and depends on various aespects such as weather or politcial issues. For example, for flights experiencing headwind, it is much more fuel-consuming, while when some policies regarding to different countries, the area is much more cost-consuming to the flight. Therefore, planning a path that optimize the trip time and cost while changing the environment is the theme of this task. 
...
The code implements an obstacle generation system with controlled density set at 0.15, which means 15% coverage. This density is appropriate balance between creating pathfinding environment while maintain the environment is solvable. Obstacles are randomly distributed across the entire map area using uniform distribution, and the count is mathematically calculated based on the total map area multiplied by the density factor. This ensures obstacles are neither too spare nor too dense, providing a realistic testing scenario for the pathfinding algorithm.
...
The implementation ensures that start and end points are randomly generated with a minimum separation of 40 units. The code continuously generates random positions until it finds a start-goal pair that satisfies the distance requirement using distance calculation math.hypot(gx - sx, gy - sy) < 40. Additionally, the validation includes checks to ensure neither point falls within the fuel-consuming area, providing comprehensive position validation before proceeding with path planning, which also meets the requirements.
...
The results are as shown above as the path, as well as the total trip time is approximately 90 minutes.
...
The group project enabled me to gain numerous collaboration and leadership skills. By overseeing the project, I learnt the positive effect of adhering to mini-deadlines, i.e finishing presentation slides within a weekend and ensuring everyone stays up to date with their tasks. I gained some coding knowledge from the coding leader, through whom I learnt to update our code in GitHub and run it to ensure it satisfies the assessment criteria. Additionally, through the lectures, I was able to acquire skills to use AI efficiently to modify and correct various code segments, making the code cleaner and easier to interpret, especially for me as a coding beginner. In managing task 2, which involved finding an optimum cost area based on set restraints (5% reduction cost along the jet stream and specific jet stream dimensions), I was able to understand how the AStar planner interprets the set code in a systematic procedure to handle the tasks, making the project overly insightful as a foundation for future projects relating to data analytics in aviation.
...
This project gives me a fantastic opportunity to work with others and a first chance to do presentations. I learned that the person who knows how to utilize Ai will dominate the future. I am the only person who knows coding in this team, and I am in charge of task 1. But my teammate already uses Ai to finish task 1 in 2 minutes to replace my job. However, finishing task 1 may take me an hour. Therefore, it is important to know how to use Ai appropriately. Besides, I am a person who does not like to plan so I am usually late in group meetings. As I am the person who does not like something to restrict me. Therefore, there are usually a lot of group mates spending time to wait me. Although they do not blame, I will feel shame. Therefore, I know that some restrictions must exist to finish the task effectively. Moreover, this project also recalls my program knowledge learned in secondary school. I oversee task 1. Although task 1 is already finished, I still want to finish it manually to provide verification to check whether the program generated by Ai is correct. It allows me to apply my math and coding knowledge to finish task 1.
