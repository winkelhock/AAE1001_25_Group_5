...
def main():
    ...
    def evaluate_scenario(name, passengers, max_flights, time_cost_level, fuel_cost_per_kg):
        print(f"\n{name}: Tbest={Tbest} min, time cost={time_cost_level}, fuel cost={fuel_cost_per_kg} $/kg")
        best_model = None
        ...
        for model, spec in airbuses.items():
            ...
            if feasible:
                ...
                print(f"{model}: flights={flights_needed}, per-flight=${pf_cost:.2f}, total=${total_cost:.2f}")
                if total_cost < best_total:
                    best_total = total_cost
                    best_model = model
            else:
                print(f"{model}: infeasible (needs {flights_needed} flights > max {max_flights})")
        if best_model is not None:
            print(f"Best: {best_model} with total cost ${best_total:.2f}")
        else:
